...
const KuisUser: React.FC = () => {
    ...
    axios.get('https://jsonplaceholder.typicode.com/albums?_limit=10')
      .then(res => {
        const mappedData = res.data.map((item: any, idx: number) => ({
          title: item.title,
          icon: idx % 3 === 0 ? 'calculate' : idx % 3 === 1 ? 'science' : 'biotech',
          info: `${10 + idx * 5} Soal • Latihan Interaktif`
        }));
  ...
  useEffect(() => {
    const filtered = categories.filter(cat => cat.title.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm py-2' : 'bg-white/80 backdrop-blur-sm py-4'}`}>
        <div className="flex justify-between items-center px-6 max-w-7xl mx-auto">
          ...
          <nav className="hidden md:flex gap-8 items-center h-full">
            {['Beranda', 'Kelas', 'Kuis', 'Tentang', 'Profil'].map((nav) => (
              <a 
                key={nav} 
                className={`text-sm font-medium font-lexend transition-colors ${nav === 'Kuis' ? 'text-blue-700 font-bold border-b-2 border-blue-700 pb-1' : 'text-slate-600 hover:text-blue-600'}`} 
                href="#"
              >
                {nav}
              </a>
          ...
          <div className="flex items-center gap-4">
            <button className="hidden md:block material-symbols-outlined text-slate-500 hover:bg-slate-50 p-2 rounded-full">logout</button>
            <div className="hidden md:flex w-8 h-8 rounded-full bg-blue-100 items-center justify-center text-blue-700 font-bold text-xs">JD</div>
            <button 
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-slate-200 shadow-lg px-6 py-4 flex flex-col gap-4 animate-in slide-in-from-top-2">
            ...
            <a className="text-slate-600 font-medium" href="#">Kelas</a>
            <a className="text-blue-700 font-bold" href="#">Kuis</a>
      ...
      <main className="pt-24 pb-20 px-6 max-w-7xl mx-auto">
        ...
        <section className="mb-10">
          <div className="mb-6">
            ...
            <p className="text-lg text-slate-600 max-w-2xl mb-6">Uji pemahamanmu dengan berbagai latihan soal dari mata pelajaran unggulan kami.</p>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div key={idx} className="group bg-white border border-slate-200 p-6 rounded-xl hover:shadow-lg hover:border-blue-700 transition-all cursor-pointer flex flex-col gap-4">
                <div className="w-12 h-12 bg-blue-50 rounded-lg flex items-center justify-center text-blue-700">
                  <span className="material-symbols-outlined text-3xl">{cat.icon}</span>
                </div>
                <h3 className="text-xl font-bold text-slate-900 capitalize line-clamp-1">{cat.title}</h3>
            ...
            <div className="text-center py-10 bg-slate-50 rounded-xl border border-slate-200">
              <p className="text-slate-500">Kuis tidak ditemukan untuk pencarian "{searchQuery}"</p>
            </div>
          )}
        </section>
