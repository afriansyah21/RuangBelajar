...
import { Search, Edit, Trash2, Loader2, BookOpen } from 'lucide-react';
...
export default function ManajemenKelasAdmin() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await axios.get<ClassItem[]>('https://jsonplaceholder.typicode.com/posts?_limit=10');
        setClasses(res.data);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  return (
    <div className="animate-[fadeUp_0.7s_ease]">
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[var(--color-primary)] mb-2 flex items-center gap-2"><BookOpen className="text-blue-500"/> Manajemen Kelas</h1>
          <p className="text-[var(--color-secondary)] text-sm">Kelola daftar kelas dan modul pembelajaran.</p>
        </div>
        <button className="primary-btn !mt-0 !w-auto px-6">Tambah Kelas</button>
      </header>
      ...
      <section className="glass-card p-6 mb-8">
        <form onSubmit={(e) => e.preventDefault()} className="flex gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari kelas..." className="w-full h-12 pl-12 pr-4 rounded-xl border border-[var(--color-border)] bg-[rgba(255,255,255,0.4)] focus:outline-none" />
          </div>
        </form>
      </section>
      ...
      {loading ? (
        <div className="flex flex-col items-center py-20">
          ...
          <p className="text-[var(--color-secondary)]">Memuat data kelas...</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[rgba(255,255,255,0.6)] text-[var(--color-secondary)] text-sm border-b border-[var(--color-border)]">
                  ...
                  <th className="p-4 font-semibold">Judul Kelas</th>
